...
# Sidebar search
st.sidebar.markdown("---")
st.sidebar.subheader("🔍 Search Books")
search_input = st.sidebar.text_input("Search by title, author, or publisher")
if search_input:
    search_results = books_df[
        books_df['title'].str.contains(search_input, case=False, na=False) |
        books_df['authors'].str.contains(search_input, case=False, na=False) |
        books_df['publisher'].str.contains(search_input, case=False, na=False)
    ][['title', 'authors', 'publisher']]
    st.sidebar.write(f"Found {len(search_results)} results:")
    st.sidebar.dataframe(search_results)
